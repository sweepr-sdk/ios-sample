...
class SSOViewController: UIViewController {
  ...
  func setupSweepr() {
    SweeprClient.updateConfig(with: userDidRequestedSSOLogoutHandler)
...
extension SSOViewController {
  ...
  func userDidRequestedSSOLogoutHandler() {
    ...
    if let logoutURL = SweeprClient.themeManager.config.ssoConfig?.logoutURL {
      SweeprClient.cloudInteraction.clearCookie(logoutURL) { (response, error) in
